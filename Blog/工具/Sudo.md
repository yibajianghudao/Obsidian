...
lastmod: 2026-02-04T15:38:40+08:00
---

## Secure Path

`secure_path` 是在 sudo`1.9.16` 版本被默认启用的,它会使用一个被认为安全的值来覆盖用户的 PATH 环境变量.通常这意味着只包含系统二进制目录.  
查看我的 `secure_path` 配置:
...
想起来我的这个路径添加到 `$PATH` 是在 `~/.zshrc` 中配置的,~~root 读不到也正常~~(这种想法是错误的,实际上 sudo 默认不会使用 root 的环境变量):
...
~~可以发现普通用户和 sudo 下都可以看到这个路径~~,普通用户可以直接运行,但是 sudo 就是找不到.  

> 实际上 `sudo echo $PATH` 并不是输出 sudo 下的环境变量,而是先在当前 shell 中解释出 `$PATH` 然后由 sudo 调用 `echo` 命令输出 (`sudo -i echo $PATH` 也一样),真正的 sudo 下的环境变量要用 `sudo env` 查看

然后发现是 [Secure Path](Sudo.md#Secure%20Path) 导致的.直接使用 sudo,环境变量会被 `secure_path` 覆盖.
查看一下 sudo 下的环境变量:
...
可以直接使用 `sudo -i` 使用 `login shell` 来获得 root 的环境变量:
