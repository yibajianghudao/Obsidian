...
tags:  
isCJKLanguage: true  
date: 2025-11-20T09:35:21+08:00  
lastmod: 2026-02-04T15:36:58+08:00
---

Zabbix 是一个开源的分布式监控软件  

## 安装  

在 [zabbix下载页面](https://www.zabbix.com/cn/download) 选择希望安装的版本  
例如,对于 ubuntu22.04-Server,Frontend,Agent2-MySQL-Apache-7.0LTS,下载步骤包括:  
(`#` 表示 root 用户命令,`##` 表示注释)  
...
> 在运行 mysql 安全安装脚本以前,需要先给 root 用户添加一个密码,否则脚本会出错,参考 [这篇文章](https://www.digitalocean.com/community/tutorials/how-to-install-mysql-on-ubuntu-22-04#step-2-how-to-configure-mysql-on-ubuntu)  
此时访问 `http://ip/zabbix` 即可访问 Web 页面  
![](assets/Zabbix/安装-20251025144935832.png)  
无法选择中文是因为还没有生成中文的 locale  
...
进入后台发现有些图表无法正常显示中文:  
![](assets/Zabbix/图表不能显示中文-20251025145841767.png)  
通过 `grep` 命令发现在 zabbix 的配置文件中指定的图形字体是 `graphfont`:  
...
![](assets/Zabbix/图表不能显示中文-20251025151539309.png)  
可以看到图表已正常显示中文  

### 自动重启  

导入一些主机之后发现 zabbix-server service 在不断的重启,部分报错日志如下:  
